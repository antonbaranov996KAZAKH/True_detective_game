...
function updateHistory(history) {
  ...
  sortedHistory.forEach((h, index) => {
    ...
    const safeTime = escapeHtml(h.time || '');

    li.style.borderLeft = '4px solid #4CAF50';
    li.style.backgroundColor = '#e8f5e9';
    li.innerHTML = `
      <div style="margin-bottom: 8px;">
        <strong style="color:#2196F3; font-size: 16px;">📍 ${safeAddress}</strong>
      </div>
      <div style="font-size: 14px; color:#333; word-break:break-word; line-height:1.5;">${displayInfo}</div>
      <div style="font-size: 11px; color:#999; margin-top: 12px;">🕒 ${safeTime}</div>
    `;
